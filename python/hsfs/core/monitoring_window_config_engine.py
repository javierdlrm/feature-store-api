...
class MonitoringWindowConfigEngine:
    ...
    def get_window_start_end_times(
        ...
        if monitoring_window_config.window_config_type not in [
            ...
            return (
                ...
                self.round_and_convert_event_time(
                    event_time=end_time, is_event_window=is_event_window
                ),
            )
        ...
        return (
            self.round_and_convert_event_time(
                event_time=start_time, is_event_window=is_event_window
            ),
            self.round_and_convert_event_time(
                event_time=end_time, is_event_window=is_event_window
            ),
    ...
    def run_single_window_monitoring(
        ...
        if (
            ...
            print(
                ...
                + " , is_event_window: "
                + (str(is_event_window) if is_event_window is not None else "None")
                + " , feature_name: "
    ...
    def fetch_entity_data_in_monitoring_window(
        ...
        try:
            if isinstance(entity, feature_group.FeatureGroup):
                # is_event_window and transformation_function don't apply here
    ...
    def round_and_convert_event_time(
        self, event_time: datetime, is_event_window: bool = False
    ) -> Optional[int]:
